// ============================================================
// aiWorker.js - Upgraded Chess AI Web Worker (Optimized)
// ============================================================
...
// Optimized Move Ordering (No game.get() calls)
function scoreMoveForOrdering(move) {
    ...
    // Utilize chess.js's built-in captured tracking for MVV-LVA
    if (move.captured) {
...
// Only used for the Slow Path now
function simulateMoveSlow(game, fen, move, color, chars, currentAbilities) {
    let childFen;
...
// QS purely uses the fast path (no abilities tracked deeply here to save time)
function quiescenceSearch(game, alpha, beta, isMaximizing, chars, abilities) {
    ...
    captures.sort((a, b) => scoreMoveForOrdering(b) - scoreMoveForOrdering(a));

    let bestVal = standPat;

    for (const move of captures) {
        game.move(move);
        const nextIsMax = game.turn() === 'w';
        const score = quiescenceSearch(game, alpha, beta, nextIsMax, chars, abilities);
        game.undo();
...
function minimaxSearch(game, depth, alpha, beta, isMaximizing, chars, abilities) {
    if (Date.now() - startTime > TIME_LIMIT_MS) throw 'timeout';
    nodeCount++;

    const fen = game.fen();
    const ttEntry = tt[fen];
    ...
    if (depth === 0) return quiescenceSearch(game, alpha, beta, isMaximizing, chars, abilities);
    ...
    for (const move of allMoves) {
        let nextGame, nextAbilities;
        let usedFastPath = false;

        const enemyColor = color === 'w' ? 'b' : 'w';
        const isDiddySlip = !move.abilityType && chars[enemyColor] === 'diddy' && abilities[enemyColor].babyOilActive;

        if (move.abilityType || isDiddySlip) {
            // SLOW PATH: Ability moves or board manipulation
            const { childFen, childAbilities } = simulateMoveSlow(game, fen, move, color, chars, abilities);
            nextGame = new Chess(childFen);
            nextAbilities = childAbilities;
        } else {
            // FAST PATH: Standard chess move mutation
            usedFastPath = true;
            game.move(move);
            nextGame = game;

            nextAbilities = { w: { ...abilities.w }, b: { ...abilities.b } };
            if (chars[color] === 'bibi') nextAbilities[color].movesSinceLastUltimate++;
            if (chars[color] === 'diddy' && !nextAbilities[color].babyOilActive) nextAbilities[color].movesSinceBabyOil++;
            if (chars[color] === 'kirk' && !nextAbilities[color].uniSniperActive) nextAbilities[color].movesSinceUniSniper++;
        }

        const score = minimaxSearch(nextGame, depth - 1, alpha, beta, nextGame.turn() === 'w', chars, nextAbilities);

        if (usedFastPath) {
            game.undo(); // Restore state!
        }
    ...
    tt[fen] = { depth, score: bestVal, flag, bestMove: bestMoveThisNode };
...
function minimaxBestMove(fen, chars, abilities, color) {
    ...
    const stdMoves = rootGame.moves({ verbose: true });
    const abilityMoves = getAbilityMoves(fen, chars, abilities, color);
    const allMoves = [...stdMoves, ...abilityMoves];
    ...
    for (let depth = 1; depth <= 20; depth++) {
        ...
        try {
            for (const move of allMoves) {
                ...
                if (move.abilityType || isDiddySlip) {
                    const { childFen, childAbilities } = simulateMoveSlow(rootGame, fen, move, color, chars, abilities);
                    nextGame = new Chess(childFen);
                    nextAbilities = childAbilities;
                } else {
                    usedFastPath = true;
                ...
                const score = minimaxSearch(nextGame, depth - 1, -Infinity, Infinity, nextGame.turn() === 'w', chars, nextAbilities);

                if (usedFastPath) rootGame.undo();
